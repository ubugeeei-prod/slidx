...
//! `@slidxjs/vite-plugin`, and a second thing that can produce a deck is two
...
fn nothing_built(out: &Path) -> String {
    format!(
        "There is nothing at {}.\n\n\
         ...
         Building a deck is @slidxjs/vite-plugin's job:\n\n\
         \x20 vite build\n\n\
...
fn not_a_deck(out: &Path) -> String {
    format!(
        "{} does not look like a built deck — nothing in it has an index.html.\n\n\
         `slidx preview` reads the output of @slidxjs/vite-plugin:\n\n\
         \x20 vite build\n",
        out.display()
    )
}
