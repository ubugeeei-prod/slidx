...
//! let found = slidx_dialect::check(&deck, &[]);
//!
...
type CheckFn = fn(&Deck, &mut Diagnostics);
...
pub fn check(deck: &Deck, allow: &[String]) -> Diagnostics {
...
mod tests {
    ...
    fn a_deck_that_says_only_things_slidx_can_carry_out_produces_nothing() {
        ...
        assert!(check(&deck(clean), &[]).is_empty(), "{:?}", check(&deck(clean), &[]));
    }
    ...
    fn the_group_name_suppresses_every_check_at_once() {
        ...
        assert!(!check(&deck(BROKEN), &[]).is_empty());
        assert!(check(&deck(BROKEN), &["dialect".to_string()]).is_empty());
    }

    #[test]
    fn one_code_can_be_suppressed_without_the_others() {
        let allow = vec!["dialect/unknown-theme".to_string()];
        let found = check(&deck(BROKEN), &allow);
    ...
    fn every_code_belongs_to_the_group_that_suppresses_it() {
        ...
        for diagnostic in check(&deck(BROKEN), &[]).iter() {
            assert!(
    ...
    fn no_finding_here_stops_a_deck_rendering() {
        ...
        assert!(!check(&deck(BROKEN), &[]).has_blocking());
    }
