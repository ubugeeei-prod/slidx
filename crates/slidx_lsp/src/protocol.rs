...
//!
//! The framing is a header block, a blank line, and a body of exactly the
//! declared number of *bytes*. The message set a language server needs is
//! closed and small. Neither justifies a dependency, and the workspace already
//! has `serde` and `serde_json` for everything below the frame.
//!
//! The one rule that is easy to get wrong: `Content-Length` counts bytes, not
//! characters. A deck full of Japanese sends three bytes per kanji, and a
//! server that counted characters would desynchronise the stream on the first
//! message that mentioned one — after which nothing works and nothing says
//! why.
...
///
/// Kept as a single shape rather than a request/response/notification trio
/// because the wire has no discriminator: which one a frame is depends on
...
pub mod error_code {
    ...
    /// Sent for every request that arrives after `shutdown`.
...
impl Message {
    ...
    pub fn error(id: RequestId, code: i64, message: impl Into<String>) -> Self {
        Self {
            jsonrpc: jsonrpc_version(),
            id: Some(id),
            error: Some(ResponseError { code, message: message.into() }),
            ..Self::default()
        }
    }
...
mod tests {
    ...
    fn an_error_response_omits_a_result() {
        let mut buffer = Vec::new();
        let message =
            Message::error(RequestId::Number(1), error_code::METHOD_NOT_FOUND, "no such method");
        write(&mut buffer, &message).unwrap();
