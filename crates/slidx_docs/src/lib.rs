...
///
/// Regenerated by `scripts/record.mjs` from real runs, and copied beside the
/// pages rather than linked from where they sit, so a published page and a
/// saved one both resolve them.
pub const MEDIA_DIR: &str = "docs/media";
...
impl Site {
    ...
    ///
    /// The media directory is copied in beside the pages. A page shows a
    /// picture one level up when it is read in the repository and alongside
    /// when it is read on the site, and the alternative to copying is a site
    /// whose images only resolve from a checkout.
    pub fn write(&self, directory: &Path) -> std::io::Result<Vec<PathBuf>> {
        ...
        let media = slidx_brand::assets::workspace_root().join(MEDIA_DIR);
        if media.is_dir() {
            written.extend(copy_into(&media, &directory.join("media"))?);
        }
...
/// Copies one directory of files, one level deep.
///
