...
import initWasm, { publishCall, type PublishCall } from "@slidx/wasm";

/**
 * Instantiates the planner.
 *
 * The wasm bytes are read from disk under Node and fetched in a browser.
 * `@slidx/wasm` is built for the web target so one artifact serves both, which
...
async function load(): Promise<void> {
  ...
  await initWasm({
    module_or_path: await readFile(require.resolve("@slidx/wasm/slidx_bg.wasm")),
  });
}
...
import type { Artifact, DeckMetadata, DeckSlide, DeckSource } from "@slidx/wasm";
