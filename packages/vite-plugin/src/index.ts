/**
 * `@slidxjs/vite-plugin` — decks in a Vite project.
 *
...
export function slidx(userOptions: SlidxOptions = {}): Plugin {
  ...
  return {
    ...
    async generateBundle(_output, bundle) {
      ...
      // The device handling, for a deck that places a camera. Asked of the
      // pages rather than of the deck model: the file exists exactly when
      // something imports it, so the two cannot disagree about whether a deck
      // needs one. A deck with no `camera:` anywhere emits nothing.
      const wantsCamera = `from "${cameraSrcFor(options)}"`;
      if (built.slides.some((slide) => slide.html?.includes(wantsCamera))) {
        this.emitFile({
          type: "asset",
          fileName: cameraFileName(options),
          source: await readCamera(),
        });
      }
      ...
      if (options.presenter) {
        this.emitFile({
          type: "asset",
          fileName: presenterRuntimeFileName(options),
          source: await readPresenterRuntime(),
        });
        this.emitFile({
          type: "asset",
...
/** The camera's own module sits beside the shared runtime. */
function cameraSrcFor(options: ReturnType<typeof resolveOptions>): string {
...
/** The device handling a declared camera imports, read once. */
function readCamera(): Promise<string> {
  camera ??= (async () => {
    const { createRequire } = await import("node:module");
    const { readFile } = await import("node:fs/promises");
    const require = createRequire(import.meta.url);

    return readFile(require.resolve("@slidxjs/runtime/camera"), "utf8");
  })();
...
function readPresenterRuntime(): Promise<string> {
  presenterRuntime ??= (async () => {
    const { createRequire } = await import("node:module");
    const { readFile } = await import("node:fs/promises");
    const require = createRequire(import.meta.url);

    return readFile(require.resolve("@slidxjs/runtime/presenter"), "utf8");
  })();
...
function readRehearsal(): Promise<string> {
  rehearsal ??= (async () => {
    ...
    return readFile(require.resolve("@slidxjs/rehearsal"), "utf8");
  })();
...
function readEffects(): Promise<string> {
  effects ??= (async () => {
    ...
    return readFile(require.resolve("@slidxjs/runtime/effects.css"), "utf8");
  })();
