 ...
 * describe the same theme `@slidxjs/theme-workshop` publishes rather than a
...
/** The document `@slidxjs/theme-workshop` ships, read from the package itself. */
...
/** A project with `@slidxjs/theme-workshop` installed the way npm would. */
async function withWorkshop(): Promise<string> {
  return project("@slidxjs/theme-workshop", {
...
describe("finding the theme packages a project installed", () => {
  it("reads the document a dependency names in its own manifest", async () => {
    ...
    expect(found[0]?.source).toBe("@slidxjs/theme-workshop");
